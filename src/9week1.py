...
# 배치 정규화 파라미터
gamma1 = np.ones((1, hidden_size_1))
beta1 = np.zeros((1, hidden_size_1))
running_mean1 = np.zeros((1, hidden_size_1))
running_var1 =np.ones((1, hidden_size_1))
epsilon = 1e-7
gamma2 = np.ones((1, hidden_size_2))
beta2 = np.zeros((1, hidden_size_2))
running_mean2 = np.zeros((1, hidden_size_2))
running_var2 =np.ones((1, hidden_size_2))
...
for epoch in range (epochs):
    ...
    for i in range(iter_per_epoch):
        ...
        da2 = dy @ W3.T
        dbn2 = da2 * ReLU_derivative(bn_output2)
        ...
        da1 = dz2 @ W2.T
        dbn1 = da1 * ReLU_derivative(bn_output1)
        ...
        dbatch_mean1 = np.sum(dz1_norm * -1 / np.sqrt(batch_var1 + epsilon), axis=0, keepdims=True) + dbatch_var1 * np.mean(-2 * (z1 - batch_mean1), axis=0, keepdims=True)

        dz1 = dz1_norm / np.sqrt(batch_var1 + epsilon) + dbatch_var1 * 2 * (z1 - batch_mean1) / batch_size + dbatch_mean1 / batch_size

        dW1 = dz1_norm / np.sqrt(batch_var1 + epsilon) + dbatch_var1 * 2 * (z1 - batch_mean1) / batch_size + dbatch_mean1 / batch_size
        ...
        W3 = SGD(W3, dW3, learning_rate)
        B3 = SGD(B3, db3, learning_rate)
